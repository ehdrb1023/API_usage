 ...
 *    실제로 성립한다는 뜻이다.
 * ✅ 2026-08-27 OpenAI 실측 — 같은 hold-out 방식으로 15일 검증.
 *    **합계 오차 0.68%**, 일별 오차율 **중앙값 0.0%**.
 *    최대 119%는 하루 $0.0032 짜리 날이라 절대 오차가 $0.004 다 — 금액이 작을수록
 *    비율이 커지는 것뿐이고 합계에는 영향이 없다.
 *    키별 안분도 `group_by=api_key_id` 실측과 대조해 **합계 오차 -0.00%** 를 확인했다.
 *    → 단가를 `quantity` 기반으로 바꾸거나 키별 비용을 실측으로 갈아탈 이유가 없다.
 */
