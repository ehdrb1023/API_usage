/**
 * lib/anthropic-rates.ts 유닛 테스트 — 순수 함수라 네트워크를 타지 않습니다.
 *
 * 핵심 검증 두 가지:
 *   1. 역산한 단가가 **공시 단가와 일치**하는가 (센트/달러 환산 실수는 100배 오차다)
 *   2. 그 단가를 KST 토큰에 곱해 만든 cost_report 위조본을 `adaptAnthropic` 에
 *      태웠을 때 금액이 다시 온전히 나오는가
 *
 * 실행:
...
import { adaptAnthropic, type AnthropicRaw, type UsageResult } from "@/lib/adapters/anthropic";
...
describe("computeTokenRates", () => {
  it("모델·토큰 종류별 단가를 공시 단가 그대로 역산한다", () => {
    const rates = computeTokenRates(dailyRaw());

    assert.equal(
      rateFor(rates, "claude-sonnet-5", "uncached_input_tokens") * MTOK,
      3,
    );
    assert.equal(
      rateFor(rates, "claude-sonnet-5", "cache_read_input_tokens") * MTOK,
      0.3,
    );
    assert.equal(rateFor(rates, "claude-sonnet-5", "output_tokens") * MTOK, 15);
    assert.equal(
      rateFor(rates, "claude-haiku-4-5", "uncached_input_tokens") * MTOK,
      1,
    );
  });
  ...
  it("토큰이 아닌 비용은 단가에서 빼고 비중만 기록한다", () => {
    ...
    const rates = computeTokenRates(raw);
    // 단가는 그대로여야 한다 — $1 이 토큰 단가에 섞이면 안 된다.
    assert.equal(
      rateFor(rates, "claude-sonnet-5", "output_tokens") * MTOK,
      15,
    );
...
describe("estimateCostResults", () => {
  it("같은 사용량을 넣으면 원래 비용이 그대로 복원된다", () => {
    const raw = dailyRaw();
    const rates = computeTokenRates(raw);
    const results = estimateCostResults(
      raw.usage_report.data[0].results as UsageResult[],
      rates,
    );

    const totalUsd = results.reduce((s, r) => s + Number(r.amount) / 100, 0);
    assert.ok(Math.abs(totalUsd - 29) < 1e-6);
  });

  it("금액 단위가 센트라 adaptAnthropic 을 통과하면 달러로 돌아온다", () => {
    const raw = dailyRaw();
    const rates = computeTokenRates(raw);
    const usageResults = [
    ...
    const point = adaptAnthropic({
      usage_report: {
        data: [
          {
            starting_at: "2026-08-25T00:00:00Z",
            ending_at: "2026-08-25T23:59:59Z",
  ...
  it("모델을 모르는 사용분은 비용을 만들지 않는다 (토큰 수에는 남는다)", () => {
    const rates = computeTokenRates(dailyRaw());
    const results = estimateCostResults(
      [{ ...usage("x", null, { output: MTOK }), model: null }],
      rates,
    );
