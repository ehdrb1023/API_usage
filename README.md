...
### Claude Code — "지금 이 세션이 얼마 쓰고 있나"

미니 창에는 벤더 두 개 말고 **Claude Code** 항목이 하나 더 뜬다. 이건 Admin API 가
아니라 이 컴퓨터의 세션 로그(`~/.claude/projects/**/*.jsonl`)를 직접 읽는다.

```
세션별      ★ 이 기능을 만든 이유. "지금 세션" 은 항상 최근 세션을 가리킨다
프로젝트별  Claude Code 가 세션을 묶는 단위 (세션을 시작한 자리)
저장소별    cwd 에서 위로 올라가 찾은 git 루트 — 모노레포의 여러 앱이 하나로 묶인다
디렉토리별  그 응답 시점의 cwd 그대로
브랜치별    gitBranch
모델별      벤더 탭과 같은 축
```

**왜 벤더 API 로는 안 되나.** `usage_report` 가 나눠 주는 축은 `model`·`api_key_id`·
`workspace_id` 뿐이다. 세션·디렉토리라는 축이 응답에 **아예 없어서** 기존 데이터를
쪼개는 방식으로는 만들 수 없다. 그래서 데이터 원천이 하나 더 붙었다.

`지금 세션` 줄은 세션 id 가 아니라 **"활성" 이라는 자리**를 가리킨다. 세션 id 로 걸어
두면 내일 빈 줄이 되지만, 이 줄은 스냅샷을 만들 때마다 가장 최근 응답이 있었던 세션으로
다시 붙는다 — 한 번 걸어 두면 계속 지금 쓰는 세션을 따라간다.

**갱신이 벤더보다 빠르다 (기본 10초).** 로컬 파일 읽기는 쿼터가 없기 때문이다.
미니 창은 로컬 항목이 걸려 있는 동안 `/api/live?scope=local` 을 따로 두드리는데,
그 경로는 벤더를 아예 부르지 않아서 Admin API 90회/시간을 건드리지 않는다.
파일도 통째로 다시 읽지 않는다 — 마지막으로 읽은 바이트 다음부터만 읽는다
(238MB·131파일 실측: 첫 스캔 243ms, 이후 12ms).

⚠️ **여기 비용은 환산치다.** 로그에 금액이 한 글자도 없어서 공개 단가표
(`lib/local/pricing.ts`)를 곱한다. 구독(Pro·Max)으로 쓰는 중이면 실제로는 과금되지
않는 금액이고, "같은 토큰을 API 로 썼다면 얼마" 라는 뜻이다. 토큰 수는 실측이다.
단가표에 없는 모델이 나오면 그 몫을 0 으로 두고 화면에 그렇게 적는다.

⚠️ **로그 한 응답이 여러 줄로 쪼개져 기록된다.** content 블록(text·tool_use…) 수만큼
줄이 생기고 모든 줄이 같은 usage 를 통째로 갖는다 (실측 중복률 49%). `(message.id,
requestId)` 로 중복을 없앤다 — 이걸 빠뜨리면 비용이 두 배로 뜬다.

이 기능은 로그 폴더가 있을 때만 켜진다. 배포 환경에는 없으므로 항목이 조용히 사라진다.
...
lib/
  ...
  ── 로컬 세션 로그 (벤더 API 아님 — 이 컴퓨터의 Claude Code) ──
  local/pricing.ts         정적 단가표. 로그에 금액이 없어서 곱한다 (환산치)
  local/scan.ts            ~/.claude/projects/**.jsonl 증분 읽기 + 중복 제거
  local/live.ts            세션·프로젝트·저장소·디렉토리·브랜치·모델 6축 집계
